...
function createUserSchema() {
	...
	UserSchema
	  .virtual('info')// info 속성에 대하여
	  .set(function(info) { // 사용자가 info 라는 칼럼을 설정하게 되면 set 호출된다.
	    var splitted = info.split(' '); // 공백으로 구분
	    this.id = splitted[0];
	    this.name = splitted[1];
	    console.log('virtual info 설정함 : %s, %s', this.id, this.name);
	  })
	  .get(function() { return this.id + ' ' + this.name }); // 저 속성을 조회하겠다.
